...
export default class EventHandler {
	...
	const demote = event.action === "demote";
		...
		const welcome = await new Canvas.Welcome()
            ...
            if (image)
                return void (await this.client.sendMessage(event.jid,welcome.toBuffer(), image, MessageType.image, {
	...
	if (bye) {
		const goodbye = await new Canvas.Goodbye()
				...
				.toAttachment();
        		return void this.client.sendMessage(event.jid, goodbye.toBuffer(), text, MessageType.image,
				{
					caption: `Goodbye *@${
						event.participants[0].split("@")[0]
					}* 👋🏻, we're probably not gonna miss you.`,
